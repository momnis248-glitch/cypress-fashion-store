...
async function notifyOrderStatus(order) {
  ...
  const messages = {
    awaiting_payment: 'Your order is waiting for payment confirmation.\nការបញ្ជាទិញរបស់អ្នកកំពុងរង់ចាំការបញ្ជាក់ការបង់ប្រាក់។',
    paid: 'Payment confirmed. Your order is now being prepared.\nបានបញ្ជាក់ការបង់ប្រាក់។ ហាងកំពុងរៀបចំការបញ្ជាទិញរបស់អ្នក។',
    shipping: 'Your order is on the way.\nការបញ្ជាទិញរបស់អ្នកកំពុងដឹកជញ្ជូន។',
    ready_for_pickup: 'Your order is ready for pickup.\nការបញ្ជាទិញរបស់អ្នករួចរាល់សម្រាប់មកយក។',
    completed: 'Your order is complete. Thank you!\nការបញ្ជាទិញរបស់អ្នកបានបញ្ចប់។ សូមអរគុណ!',
...
function productInput(input) { const name_en = String(input.name_en || '').trim(), price = Number(input.price), category = ['clothes', 'bags', 'charms'].includes(input.category) ? input.category : 'clothes', sizes = Array.isArray(input.sizes) ? input.sizes : String(input.sizes || '').split(','), colors = Array.isArray(input.colors) ? input.colors : String(input.colors || '').split(','); if (!name_en || !Number.isFinite(price) || price < 0) throw Error('Product name and price are required.'); return { name_en, name_km: String(input.name_km || '').trim(), description_en: String(input.description_en || '').trim(), description_km: String(input.description_km || '').trim(), category, price, sizes: category === 'clothes' ? sizes.map(size => String(size).trim()).filter(Boolean).slice(0, 20) : [], colors: colors.map(color => String(color).trim()).filter(Boolean).slice(0, 30), published: input.published !== false, featured: input.featured === true }; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/orders') { const input = await readBody(req), telegramUser = verifyTelegramInitData(input.telegramInitData), webOrder = !input.telegramInitData; if (!telegramUser && !webOrder) return sendJson(res, 401, { error: 'Telegram verification failed.' }); if (!Array.isArray(input.items) || !input.items.length || !Number.isFinite(input.total)) return sendJson(res, 400, { error: 'Invalid order.' }); const order = { order_number: `CYP-${randomUUID().slice(0, 8).toUpperCase()}`, telegram_user_id: String(telegramUser?.id || `web-${randomUUID()}`), customer_name: String(input.name || '').trim(), contact: String(input.contact || '').trim(), address: String(input.address || '').trim(), delivery: input.delivery === 'pickup' ? 'pickup' : 'delivery', region: String(input.region || ''), items: input.items, subtotal: Number(input.subtotal), shipping: Number(input.shipping), total: Number(input.total), status: 'awaiting_payment' }; if (!order.customer_name || !order.contact) return sendJson(res, 400, { error: 'Customer information is required.' }); await db('orders', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(order) }); if (telegramUser) await sendPaymentQr(telegramUser.id, order); return sendJson(res, 201, { order_number: order.order_number, payment_qr_url: telegramUser ? '' : (process.env.PAYMENT_QR_IMAGE_URL || '/payment-qr.png') }); }
