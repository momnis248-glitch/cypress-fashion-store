...
function productInput(input) { const name_en = String(input.name_en || '').trim(), price = Number(input.price), category = ['clothes', 'bags', 'charms'].includes(input.category) ? input.category : 'clothes', sizes = Array.isArray(input.sizes) ? input.sizes : String(input.sizes || '').split(','); if (!name_en || !Number.isFinite(price) || price < 0) throw Error('Product name and price are required.'); return { name_en, name_km: String(input.name_km || '').trim(), description_en: String(input.description_en || '').trim(), description_km: String(input.description_km || '').trim(), category, price, sizes: category === 'clothes' ? sizes.map(size => String(size).trim()).filter(Boolean).slice(0, 20) : [], published: input.published !== false, featured: input.featured === true }; }
