...
async function uploadProductVideo(dataUrl) {
  const match = /^data:(video\/(?:mp4|webm));base64,([A-Za-z0-9+/=]+)$/.exec(dataUrl || '');
  if (!match) throw Error('Use an MP4 or WebM video.');
  const data = Buffer.from(match[2], 'base64');
  if (data.length > 10 * 1024 * 1024) throw Error('Product video must be 10 MB or smaller.');
  const extension = match[1] === 'video/mp4' ? 'mp4' : 'webm';
  const name = `${randomUUID()}.${extension}`;
  const response = await fetch(`${process.env.SUPABASE_URL}/storage/v1/object/products/${name}`, { method: 'PUT', headers: { apikey: process.env.SUPABASE_SERVICE_ROLE_KEY, authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`, 'content-type': match[1], 'x-upsert': 'false' }, body: data });
  if (!response.ok) throw Error('Video upload failed.');
  return `${process.env.SUPABASE_URL}/storage/v1/object/public/products/${name}`;
}
...
async function productImages(input) {
  ...
  const supplied = [...uploads, ...legacy].filter(Boolean).slice(0, 8);
