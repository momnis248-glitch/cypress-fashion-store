...
(function () {
  ...
  paymentOrdersAdmin = function () {
    ...
    return `<section class="panel payment-orders"><div class="section-head"><h2>Orders</h2><small>${orders.length} orders</small></div><div class="payment-order-list">${orders.map(order => `<article class="payment-order-row"><div><b>${html(order.order_number)}</b><small>${new Date(order.created_at).toLocaleString()}</small><span>${html(order.customer_name)} · ${html(order.contact)}</span></div><div><strong>${money(order.total)}</strong><span class="order-status status-${html(order.status)}">${label(order.status)}</span>${order.payment_proof_file_id ? '<small class="proof-flag">付款凭证已上传</small>' : ''}</div><button onclick="openOrderDetail('${order.id}')">查看详情</button></article>`).join('') || '<div class="empty">暂无订单。</div>'}</div></section>`;
  };
